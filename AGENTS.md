...
- CI de app: `.github/workflows/ci.yml` verifica main/PR/manual con datos sintéticos y sin secretos productivos. `.github/workflows/deploy-app.yml` (Publicar VPS) publica los destinos habilitados tras CI exitoso de main, con `VPS_DEPLOY_ENABLED` y flags de app/landing, con environment production, SSH restringido y rollback de código; cambios de migraciones requieren revisión/manual. Seguir `docs/deploy/CI-Y-ENTORNO.md`. El entorno VPS se conserva fuera del repositorio, en `/etc/openfunnel/app.env` (0600). No imprimir `docker compose config` ni `docker inspect` completos con secretos. Los informes privados de infraestructura viven en `docs/qa/private/`, ignorado por Git.
