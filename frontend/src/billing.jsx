  ...
  return <><div className="page-heading"><div><h1>Facturación</h1><p className="muted">Tu plan y el uso de tus agentes.</p></div>{subscription && <button className="button secondary" disabled={!!busy} onClick={()=>redirect('portal',{})}>{busy==='portal'?'Abriendo…':'Gestionar suscripción ↗'}</button>}</div>
    ...
    <LoadState state={state}>{data && <>
      ...
      {!data.enabled ? <Notice>Esta instancia funciona sin cobros de OpenFunnel.</Notice> : <>
        {window.location.hash.includes('checkout=success') && !data.period && <Notice>Estamos esperando la confirmación del pago. Tu saldo se actualizará automáticamente.</Notice>}
        ...
        {!data.period && <Notice>{subscribed?'Aún no hay un período pagado vigente. Revisa el estado de tu pago en el portal.':'Elige un plan para comenzar a usar tus agentes.'}</Notice>}
        {data.period?.available===0 && <Notice error>Agotaste los créditos del período. No pueden iniciarse nuevos usos del agente. Tras renovar, retoma las conversaciones pausadas; puedes seguir gestionando tus recursos.</Notice>}
        {!subscribed && <section aria-labelledby="billing-plans-title"><h2 id="billing-plans-title">Planes disponibles</h2>
      ...
      <section className="billing-history"><h2>Actividad de créditos</h2><LoadState state={history}>{history.data && <>
        {!history.data.items.length ? <p className="muted">Todavía no hay movimientos de créditos.</p> : <div className="table-scroll"><table><thead><tr><th>Fecha</th><th>Operación</th><th>Origen</th><th>Estado</th><th>Créditos</th></tr></thead><tbody>{history.data.items.map(r=><tr key={r.operation_key}><td>{when(r.created_at)}</td><td>{r.kind==='llm'?'Llamada al modelo':'Herramienta'}</td><td>{{agent:'Agente',test:'Prueba',validation:'Validación'}[r.source] || r.source}</td><td>{statusLabels[r.status]}</td><td>{r.status==='released'?'0':'1'}</td></tr>)}</tbody></table></div>}
...
function BillingAdmin({setDirty,confirm}) {
  ...
  return <><div className="page-heading"><div><h2>Facturación de la instancia</h2><p className="muted">Tus productos de Polar, con los créditos que tú defines.</p></div><a className="button secondary" href="https://polar.sh/dashboard" target="_blank" rel="noreferrer">Abrir Polar ↗</a></div>
    <Notice error>{error}</Notice><Notice>{notice}</Notice><LoadState state={state}>{data && <>
      ...
      <section className="billing-history"><h2>Suscripciones y consumo</h2><div className="table-scroll"><table><thead><tr><th>Cuenta</th><th>Suscripción</th><th>Disponibles</th><th>Consumidos</th><th>Fin del período</th></tr></thead><tbody>{data.accounts.map(a=><tr key={a.workspace_id}><td>{a.name}<br/><small>{a.email}</small></td><td>{a.subscription?statusLabels[a.subscription.status] || a.subscription.status:'Sin suscripción'}{a.subscription?.cancel_at_period_end?' · No renueva':''}</td><td>{number(a.period?.available)}</td><td>{number(a.period?.consumed)}</td><td>{when(a.period?.ends_at)}</td></tr>)}</tbody></table></div><Pager page={page} total={data.total} onChange={setPage}/></section>
