...
if [[ "${SSH_ORIGINAL_COMMAND:-}" =~ ^deploy-landing\ ([0-9a-f]{40})\ ([0-9]{1,20})$ ]]; then
  exec /usr/bin/sudo -n /usr/local/sbin/openfunnel-deploy "${BASH_REMATCH[1]}" "${BASH_REMATCH[2]}" landing
fi
